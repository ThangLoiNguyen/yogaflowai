    ...
    <main className="flex-1 bg-slate-950">
      <div className="mx-auto flex w-full max-w-6xl flex-col px-4 pb-14 pt-8 sm:px-6 lg:px-8">
        <header className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <Badge className="bg-sky-500/15 text-sky-200 border-sky-400/40 dark:text-sky-300">
          ...
          <div className="flex gap-2 items-center">
            ...
            <form action={async () => {
              ...
              <Button
                ...
                size="sm"
                variant="outline"
                className="border-rose-300 bg-rose-50 text-rose-700 hover:bg-rose-100 dark:border-rose-700 dark:bg-rose-500/10 dark:text-rose-200 dark:hover:bg-rose-500/20"
              >
                Đăng xuất
              </Button>
            </form>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,2.1fr)_minmax(0,1.2fr)]">
          ...
          <div className="space-y-6">
            <Card className="border-sky-500/30 bg-white/80 dark:bg-slate-900/80">
              <CardHeader>
                <CardTitle className="flex items-center justify-between text-sm font-medium text-slate-800 dark:text-slate-200">
                  Lớp học đề xuất
                  <span className="text-[11px] text-sky-500 dark:text-sky-300">
              ...
              <CardContent className="space-y-4">
                ...
                <Link href="/recommendation">
                  <Button
                     variant="ghost"
                     size="sm"
                     className="mt-1 w-full justify-start text-xs text-sky-600 hover:bg-slate-100 hover:text-sky-700 dark:text-sky-300 dark:hover:bg-slate-900 dark:hover:text-sky-400"
                  >
                    Xem thêm đề xuất
                  </Button>
                </Link>
            ...
            <Card className="border-slate-200 bg-white/80 dark:border-slate-800 dark:bg-slate-900/80">
              ...
              <CardContent className="space-y-3">
                {upcomingClasses.length === 0 ? (
                  <p className="text-xs text-slate-500 dark:text-slate-400">Chưa có lớp học nào. Hãy đăng ký từ gợi ý bên trên!</p>
                ) : (
                  upcomingClasses.map((cls) => (
                    <div
                      key={cls.id}
                      className="flex items-center justify-between rounded-md border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs dark:border-slate-800 dark:bg-slate-900"
                    >
                      <div>
